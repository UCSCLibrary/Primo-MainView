...
app.controller('LoansAfterController', ['$scope', '$element', function($scope, $element){
  ...
  this.$onInit = ()=> {
    ...
    // TODO: Figure out how to tap into the asnyc process in Primo, until then interval!
    var loansInterval = window.setInterval(function(){
      loans = vm.loansService._loansList.active;
      loanCount++;
      // Stop checking if we find loans or 10 seconds has passed
      if ((loans.length) || (loanCount > 20)) {
        processLoanRenewButtons(loans);
        clearInterval(loansInterval);
      }
    }, 500);
  };

  function processLoanRenewButtons($loans) {
    var prmLoans = $element[0].parentNode.getElementsByTagName('prm-loan'); // get all <prm-loan> inside <prm-loans> parent
    var loanCount = 0; // To track which loan we're inspecting
    angular.forEach($loans, function(item) {
      if ((item.mainlocationcode == "RES_SHARE") && (item.renew == "N")) {
        ...
        prmLoans[loanCount].getElementsByClassName('list-item-actions').item(0).innerHTML = `
          <div class="renewable">
